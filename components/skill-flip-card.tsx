  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`absolute inset-0 w-full h-full rounded-2xl bg-gradient-to-br ${colors.gradient} border ${colors.border} transition-all duration-300 shadow-lg ${colors.shadow} p-6 flex flex-col backdrop-blur-sm`}
          style={{
            backfaceVisibility: "hidden",
            transform: "rotateX(180deg)",
          }}
        >
          <h3 className={`font-bold mb-4 ${colors.text} text-sm uppercase tracking-wider`}>
            {title}
          </h3>
          <div className="flex flex-wrap gap-2 flex-1 content-start overflow-y-auto">
            {skills.map((skill) => (
              <Badge
                key={skill}
                variant="secondary"
                className={`${colors.badgeBg} text-white border-white/10 hover:border-white/20 transition-colors`}
              >
                {skill}
              </Badge>
            ))}
